...
XStatus ETHERNETCORE_MM_Reg_SelfTest(void * baseaddr_p)
{
	...
	for (write_loop_index = 0 ; write_loop_index < 4; write_loop_index++)
	  ETHERNETCORE_MM_mWriteReg (baseaddr, write_loop_index*4, (write_loop_index+1)*READ_WRITE_MUL_FACTOR);
	for (read_loop_index = 0 ; read_loop_index < 4; read_loop_index++)
	  if ( ETHERNETCORE_MM_mReadReg (baseaddr, read_loop_index*4) != (read_loop_index+1)*READ_WRITE_MUL_FACTOR){
...
XStatus ETHERNETCORE_MM_Mem_SelfTest(void * baseaddr_p)
{
	...
	{
	  ETHERNETCORE_MM_mWriteMemory(baseaddr+4*Index, (0xDEADBEEF % Index));
	}

	for ( Index = 0; Index < 16; Index++ )
	{
	  Mem32Value = ETHERNETCORE_MM_mReadMemory(baseaddr+4*Index);
